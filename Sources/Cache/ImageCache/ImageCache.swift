...
//
//
//  Created by Aleksey Goncharov on 7.3.24..
//
...
struct CacheStoreResult {
    ...
    let memoryCacheResult: Result<Void, Never>
    ...
    let diskCacheResult: Result<Void, KingfisherError>
}
...
enum ImageCacheResult {
    ...
    var image: KFCrossPlatformImage? {
        switch self {
        case let .disk(image): return image
        case let .memory(image): return image
...
class ImageCache {
    ...
    let memoryStorage: MemoryStorage.Backend<KFCrossPlatformImage>
    ...
    typealias DiskCachePathClosure = (URL, String) -> URL
    ...
    func store(_ image: KFCrossPlatformImage,
               original: Data? = nil,
               forKey key: String,
    ...
    private func syncStoreToDisk(
        ...
        let result: CacheStoreResult
        do {
            try diskStorage.store(value: data, forKey: computedKey, expiration: expiration, writeOptions: writeOptions)
        ...
        loadingQueue.execute {
            do {
                ...
                if options.backgroundDecode {
                    image = image?.decoded(scale: options.scaleFactor)
                }
